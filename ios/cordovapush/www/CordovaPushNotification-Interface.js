    ...
    if(cordovapushapp.deviceType === "android"){
        ...
        window.plugins.pushNotification.registerDevice({
            ...
            document.addEventListener('push-notification', function(event) {                   
                cordovapushapp.onmessage(event.notification);
                window.plugins.pushNotification.setApplicationIconBadgeNumber(event.notification.aps.badge, function(status) {
...
function C2DM_Event(e) {
	...
	case 'registered':
		...
		break;
	case 'message':
		cordovapushapp.onmessage(JSON.parse(e.data));
		break;
	case 'error':
